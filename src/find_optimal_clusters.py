...
def analyze_optimal_clusters(
    article_vectors, k_range=None, output_dir="assets", save_results=True
):
    """
    ...
    silhouette_plot_path = Path(output_dir) / "silhouette_plot.html"
    silhouette_results = silhouette_method(
        article_vectors, k_range=k_range, output_file=silhouette_plot_path
    )

    # Save results to JSON for later use
    if save_results:
        results_file = Path(output_dir) / "cluster_analysis_results.json"
        results_file.write_text(json.dumps(silhouette_results, indent=2))
...
def main(summary_dir: str, output_dir: str = "assets", k_range: str = None):
    ...
    results = analyze_optimal_clusters(
        article_vectors, k_range=k_range, output_dir=output_dir, save_results=True
    )
...
Examples:
  ...
  uv run python src/find_optimal_clusters.py --summary_dir summaries_deepseek-r1_trial1
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Range of k values to test. Format: '2-15' or '2,3,4,5,6' "
            "(default: 2 to min(15, n_articles-1))"
        ),
    )
